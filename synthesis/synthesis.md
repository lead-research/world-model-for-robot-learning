...
# World Model for Evaluation 分类综述
...
| 阶段 | 时间 | 代表性工作 | 核心特征 |
|------|------|-----------|---------|
| 萌芽期 | 2023-2024 | UniPi, GR-1, DayDreamer, OXE, DROID | 视频生成→机器人；数据里程碑；世界模型用于RL |
| 爆发期 | 2024-2025 | IDM-style全家桶, Latent-space, Video Generation | 方法多样化；各路线独立探索 |
| 收敛期 | 2025-2026 | Unified VLA, MoE/MoT, RL+WM, Evaluation | 评估驱动；功能化；物理一致性关注 |
| 物理化 | 2026+ | ABot-PhysWorld, Vid2World, RoVid-X, Action100M | 物理标注；自动化提取；互联网视频利用 |

### 核心趋势判断

1. **从视觉逼真到物理正确**: 评估基准（WorldArena, RBench）推动目标转变
2. **从生成像素到隐空间预测**: JEPA-style 因效率优势成为长期方向
3. **从单一方法到功能化世界模型**: 世界模型作为仿真器/评估器/数据引擎
4. **从实验室数据到规模化+自动化**: 数据瓶颈迫使社区探索互联网视频和自动化标注
5. **从短程到长程+错误恢复**: 长程操作是未解决的硬问题

### 评估体系总结

| 基准 | 维度 | 核心发现 | 对长程操作的意义 |
|------|------|---------|---------------|
| EVA-Bench | 反思+视频预测 | 反思提升预测质量 | 长程任务需要自我校正 |
| WorldSimBench | 世界仿真器质量 | 人类偏好高度相关 | 评估需包含人类判断 |
| EWMBench | 场景+运动+语义 | 多维度细粒度评估 | 语义一致性对语言指令重要 |
| DreamGen Bench | 通用化评估 | 视频世界模型提升泛化 | 泛化是长程的前提 |
| World-in-World | 闭环世界评估 | 闭环验证世界模型 | 长程必须闭环评估 |
| WoW-World-Eval | 执行可执行性 | 大多数模型执行崩溃 | **可执行性是关键** |
| RBench | 物理合理性 | 物理真实感严重不足 | **物理一致性是瓶颈** |
| WorldArena | 感知-功能鸿沟 | 高视觉≠高功能 | **功能效用优先** |

### 数据集体系总结

| 数据集 | 规模 | 独特价值 | 适用场景 |
|--------|------|---------|---------|
| OXE | 100万+ | 跨具身标准化 | 通用预训练 |
| DROID | 2.4万 | 真实场景多样性 | 真实环境泛化 |
| AgiBot-World | 100万+ | 标准化+灵巧手 | 规模化操作 |
| Galaxea | 未披露 | 人居/子任务 | 家庭长程 |
| Action100M | 1亿 | 互联网自动提取 | 视觉预训练 |
| RoVid-X | 400万 | 物理属性标注 | 物理世界模型 |
| EVA-Instruct | 偏好对 | 反思/偏好 | RLHF后训练 |
| HF-Embodied | 偏好对 | 人类评估基准 | 评估验证 |

### 未来研究方向

**短期（1-2年）**: 物理一致性世界模型、评估驱动优化、实时世界模型
**中期（2-3年）**: 组合式长程世界模型、开放环境自适应、人机协作
**长期（3-5年）**: 通用物理世界模型、世界模型即操作系统、神经符号世界模型

---

*最终整合综述日期: 2026-06-09 | 全部 115 篇论文完成*
